...
def compute_hsic_mi_matrix_timer_batch(
    model: TimerModel,
    device: torch.device,
    ...
    *,
    ...
    """
    Same HSIC / MI-proxy computation as the main training loop in this file: one forward
    (joint concat or split encode) then HSIC(h_x[t], z_y) per history patch and layer.

    Expects batch_x [B, seq_len, M], batch_y in IMS shape [B, label_len + pred_horizon, M]
    when use_ims (pred_horizon = output_len on test). For multivariate M>=1; HSIC needs
    enc_embedding batch width >= 4 (typically B*M >= 4, e.g. B=1 and M>=4).

    Returns:
        hsic_batch: [n_layers, n_x] with n_x history patches used for HSIC.
        forward_mode: description string (joint_concat or split_*).
    """
    model.eval()
    ...
    y_future = _extract_y_future(batch_y, use_ims, label_len, pred_horizon)
    ...
    if n_x_standalone <= 0 or n_y_standalone <= 0:
        raise ValueError(
            f"No patches: seq_len={seq_len}, L_fut={L_fut}, patch_len={patch_len}, stride={stride}"
        )
    ...
    with torch.no_grad():
        ...
        if not has_mixed and hist_idx and fut_idx:
            ...
            if dec_xy.shape[0] < 4:
                raise ValueError(
                    f"HSIC needs enc_embedding batch width >= 4 (got {dec_xy.shape[0]}). "
                    "Use multivariate data (features M with M>=4) or a larger batch."
                )
            handles_j = register_encoder_layer_hooks(model, h_all)
            _ = model.decoder(dec_xy)[0]
            remove_hooks(handles_j)
            if len(h_all) != n_layers:
                raise RuntimeError(f"Hook layers {len(h_all)} != n_layers {n_layers}")
            ...
            if dec_y.shape[0] != dec_in.shape[0] or n_vars_y != n_vars:
                raise RuntimeError("Encoder shape mismatch between history and future branch.")
            if dec_in.shape[0] < 4:
                raise ValueError(
                    f"HSIC needs enc_embedding batch width >= 4 (got {dec_in.shape[0]}). "
                    "Use multivariate data (features M with M>=4) or a larger batch."
                )
    ...
    if len(hx) != n_layers or len(hy) != n_layers:
        raise RuntimeError("Forward did not populate encoder hooks for all layers.")

    n_hx = hx[0].shape[1]
    n_hy = hy[0].shape[1]
    if n_x > n_hx or n_y_tokens > n_hy:
        raise RuntimeError(
            f"Patch index range mismatch: n_x={n_x}, n_hx={n_hx}, n_y_tokens={n_y_tokens}, n_hy={n_hy}"
        )

    hsic_batch = compute_hsic_matrix_per_input_patch(hx, hy, n_layers, n_x)
    if np.all(np.isnan(hsic_batch)):
        raise ValueError(
            "HSIC is all NaN (e.g. too few samples for median kernel). "
            "Try multivariate ETTh with M>=4 and batch 1, or align seq_len/patch_len with the checkpoint."
        )
    return hsic_batch, forward_mode
