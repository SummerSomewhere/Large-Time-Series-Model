...
Performs visualization and similarity analysis of Timer representations:
  ...
  1. UMAP / PCA Visualization:
     - Project specific layers (e.g., 0, 6, 12) to 2D
     - Color points by true generative parameter (e.g., frequency, slope)
     - Observe smooth parameter flow across embedding space

  2. CKA (Centered Kernel Alignment):
     - Compute CKA(H^(l1), H^(l2)) between all pairs of layers
     - Output similarity matrix heatmap
     - Observe how concept representations evolve with depth

  3. Disentanglement Analysis:
     - t-SNE / UMAP of pooled representations per layer
     - Color by concept type and by parameter values
     - Quantify separation using silhouette score

Usage:
    python experiments/ts_concept_multidim_analysis.py \
...
def cka_gaussian(X: np.ndarray, Y: np.ndarray, sigma_X: float = None, sigma_Y: float = None) -> float:
    ...
    Args:
        ...
        sigma_X: bandwidth for X kernel (None = median heuristic)
        sigma_Y: bandwidth for Y kernel (None = median heuristic)
    ...
    # Centering: H @ K @ H where H = I - 1/n
    H = np.eye(n) - np.ones((n, n)) / n
    Kc = H @ K @ H
    Lc = H @ L @ H

    # HSIC = trace(Kc @ Lc) / (n^2 - n)
    hsic = np.trace(Kc @ Lc)
...
def compute_cka_matrix(layer_reps: list[np.ndarray], max_samples: int = 2000) -> np.ndarray:
    ...
    Args:
        layer_reps: list of [n, d] arrays, one per layer
...
def plot_cka_heatmap(cka_matrix: np.ndarray, output_path: str):
...
def plot_layer_embeddings(
    ...
    plt.suptitle(f"Layer Embeddings ({method.upper()}, colored by concept)", fontsize=12, fontweight="bold")
...
def plot_param_flow(
    ...
    # Find which concept(s) have this param
    if concept:
    ...
    for row, tc in enumerate(target_concepts):
        ...
        tc_rep = layer_reps[0]  # use layer 0 for param extraction
        tc_rep = tc_rep[tc_idx]
        ...
        for l, rep in enumerate(layer_reps):
            ...
            scatter = ax.scatter(
                emb[:, 0], emb[:, 1],
                c=pv_sub, cmap="coolwarm", s=10, alpha=0.7,
            )
            ax.set_xticks([])
            ax.set_yticks([])
    ...
    plt.suptitle(
        f"Parameter Flow: {param_name} ({method.upper()})",
...
def plot_silhouette_scores(
    ...
    ax.set_title("Concept Disentanglement: Silhouette Score per Layer", fontsize=12, fontweight="bold")
...
def compute_procrustes_similarity(
    ...
    Measures how much the representation geometry changes.
    """
...
def plot_procrustes_similarity(
    ...
    ax.set_title("Representation Continuity Between Layers", fontsize=12, fontweight="bold")
    ax.set_xticks(layers)
...
def main():
    ...
    print("=" * 60)
    print("Step 4: Multi-Dimensional Analysis")
    print("=" * 60)
    ...
    if os.path.exists(combined_path):
        ...
        ckpt = torch.load(os.path.join(args.rep_dir, layer_files[0]), map_location="cpu")
        if "params" in ckpt:
            params = ckpt["params"]
            concept_idx = ckpt["concept_idx"].numpy()
            concepts = ckpt["concepts"]
    ...
    if not args.no_plot:
        plot_cka_heatmap(cka_matrix, os.path.join(args.output_dir, "cka_heatmap.png"))
    ...
    if not args.no_plot and not args.cka_only:
        ...
        # Determine which params to visualize based on available concepts
        param_concept_pairs = [
            ("phi", "AR1"),
            ("mu", "RandomWalk"),
    ...
    with open(summary_path, "w") as f:
        f.write("Multi-Dimensional Analysis Summary\n")
        f.write("=" * 40 + "\n\n")
