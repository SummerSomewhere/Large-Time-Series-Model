#!/bin/sh
#
# ETTh1.sh-style weather forecast + Geometric-HPE with feature-only curvature:
#   v' = v * (1 + a * k_norm),  PE not scaled (geometric_hpe_pe_curv_weighted=0).
#
# Default flow (RUN_FINETUNE=1): run.py --is_finetuning 1 → exp.finetune() then exp.test()
# (test metrics under ./test_results/), then draw L×H self-attention figure.
#
# Only figure (skip train/test): RUN_FINETUNE=0 CKPT_PATH=.../checkpoint.pth bash ...
#
...
# ========= same layout as scripts/forecast/ETTh1.sh =========
model_name=Timer
seq_len=672
label_len=576
pred_len=96
...
# 1 = finetune + test (run.py), then figure; 0 = load CKPT_PATH and only plot
...
OUT_DIR="${OUT_DIR:-./figure_attention_geo_layers_heads}"
OUT_NAME="${OUT_NAME:-attention_layers_heads.png}"
BATCH_INDEX="${BATCH_INDEX:-0}"
BATCH_EFF_INDEX="${BATCH_EFF_INDEX:-0}"
METRICS_MAX_BATCHES="${METRICS_MAX_BATCHES:-0}"

# Finetune id (used when RUN_FINETUNE=1)
model_id="${MODEL_ID:-etth1_geo_v1ak_pe0}"
...
if [ "$RUN_FINETUNE" = "1" ]; then
  ...
  echo "========== [1/2] Finetune then test (run.py: exp.finetune → exp.test) =========="
  # Feature-only curvature; PE not enhanced (pe_curv_weighted=0)
...
if [ "$RUN_FINETUNE" = "1" ]; then
  echo "========== [2/2] Attention figure (L×H heatmaps) =========="
else
  echo "========== Attention figure only (RUN_FINETUNE=0) =========="
fi
echo "CKPT_PATH=$CKPT_PATH"

python figure_geo_attention_layers_heads.py \
  --ckpt_path "$CKPT_PATH" \
  --root_path "$root_path" \
  --data_path "$data_csv" \
  --data "$data" \
