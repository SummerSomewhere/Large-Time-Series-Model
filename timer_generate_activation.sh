...
# 创建输出目录
mkdir -p "$OUTPUT_DIR/reasoning_evolve"

# 运行激活提取脚本
python MI-Peaks/src/timer_generate_activation.py \
    --root_path "./datasets/" \
    --data_path "$DATA_PATH" \
    --data "$DATA" \
    --features "M" \
...
echo ""
echo "输入序列激活提取完成！"
echo "输出文件: $OUTPUT_DIR/reasoning_evolve/${DATA}_timer_mi.pth"
