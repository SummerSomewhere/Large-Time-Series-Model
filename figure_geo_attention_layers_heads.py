#!/usr/bin/env python3
"""
Geometric-HPE Timer: encoder self-attention heatmaps for every layer and head.
Patch embed uses v' = v * (1 + a * k_norm) when trained with geometric_hpe_linear_k_patch_scale=1;
PE is not scaled by k when geometric_hpe_pe_curv_weighted=0 (default here).
...
def curvature_per_patch_normalized(
    ...
    """Same k as GeometricHPEPatchEmbedding: mean |Δ²x| per patch after normalization."""
    b, l, m = batch_x.shape
...
def build_config_ns(args: argparse.Namespace) -> SimpleNamespace:
    """Geo Timer with v*(1+a*k_norm); PE not scaled when pe_curv_weighted=0."""
    pe_w = int(getattr(args, "geometric_hpe_pe_curv_weighted", 0))
    ns = SimpleNamespace(
        ...
        geometric_hpe_linear_k_a_init=float(
            getattr(args, "geometric_hpe_linear_k_a_init", 0.01)
        ),
        ...
        geometric_hpe_pe_proj_ones_input=int(
            getattr(args, "geometric_hpe_pe_proj_ones_input", 0)
        ),
        geometric_hpe_only_linear_trainable=int(
            getattr(args, "geometric_hpe_only_linear_trainable", 0)
        ),
    )
    return ns
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Geo-HPE Timer: L×H self-attention maps (v·(1+a·k), PE not scaled if pe_curv_weighted=0)"
    )
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Checkpoint (.ckpt or .pth) trained with geometric_hpe + linear_k (feature curvature).",
    )
    ...
    p.add_argument("--output_dir", type=str, default="./figure_attention_geo_layers_heads")
    p.add_argument("--output_name", type=str, default="attention_layers_heads.png")
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="0: PE not scaled by k (feature-only v·(1+a·k_norm)); 1: also PE·(1+b·k_norm).",
    )
...
def main():
    ...
    k = curvature_per_patch_normalized(batch_x, cfg.patch_len, stride, padding=0)
    ...
    if n_from_cfg != n_tok:
        raise SystemExit(
            f"Inconsistent patch count: N={n_tok} vs ({args.seq_len},{args.patch_len})→{n_from_cfg}"
        )

    print("IMS test metrics (Geo-HPE)...", flush=True)
    ...
    # Wider figure for many heads/layers
    fig_w = max(14.0, 1.1 * n_heads * 1.15)
    fig_h = max(10.0, 1.0 * n_layers * 1.05)
    fig = plt.figure(figsize=(fig_w, fig_h + 2.2), dpi=args.dpi)
    gs = fig.add_gridspec(
        2,
        1,
    ...
    pew = int(args.geometric_hpe_pe_curv_weighted)
    pe_note = (
        "PE not scaled by k (pe_curv_weighted=0)"
        if pew == 0
        else "PE scaled: PE·(1+b·k_norm)"
    )
    fig.suptitle(
        r"Geometric-HPE — $v'=v(1+a\cdot k_{\mathrm{norm}})$ + harmonic PE  |  "
        + pe_note,
        fontsize=11.5,
        fontweight="bold",
        y=0.98,
    ...
    k_line = "Per-patch k (mean |Δ²x|): " + "  ".join(k_parts)
    foot = (
        k_line
        + "\n\n"
        + f"IMS test: MSE={mse_v:.6f}, MAE={mae_v:.6f}  |  ckpt: {ckpt}"
    )
    ...
    meta = {
        ...
        "ckpt_path": ckpt,
        "geometric_hpe_pe_curv_weighted": pew,
        "note": "Feature curvature v·(1+a·k_norm); PE not enhanced when pe_curv_weighted=0.",
