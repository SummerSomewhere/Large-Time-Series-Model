...
def load_backbone_state_dict_shape_safe(
    backbone: torch.nn.Module, sd: dict, *, log_prefix: str = ""
) -> tuple[int, int]:
    """
    Copy checkpoint tensors into backbone only when names and shapes match.

    Use when finetuning with a different patch_len (or other shape drift): patch embedding and
    projection layers are skipped; transformer blocks load if shapes still align.
    """
    ...
    n_skip = len(skipped_mismatch)
    if skipped_mismatch:
        msg = (
            f"{log_prefix}Skipped {n_skip} checkpoint key(s) due to shape mismatch "
            f"(e.g. patch_len differs from pretrained): "
            + "; ".join(skipped_mismatch[:8])
            + ("; ..." if len(skipped_mismatch) > 8 else "")
        )
        warnings.warn(msg, UserWarning, stacklevel=2)
    return n_loaded, n_skip
